...
PASSWORD = os.environ.get("PANEL_PASS", "112233")
...
# ---------- Global State (per warm instance) ----------
session_cookie = None
sesskey = None
last_login = 0
login_lock = Lock()
...
# ---------- COMPLETE COUNTRY MAP (from original app (2).js) ----------
COUNTRY_MAP = {
...
def get_cookie_from_headers(headers):
    ...
    if not set_cookie:
        return None
    first = set_cookie[0] if isinstance(set_cookie, list) else set_cookie
    match = re.match(r'^([^=]+)=([^;]+)', first)
    return f"{match[1]}={match[2]}" if match else None

# ---------- Helper: Clean phone number with country detection ----------
...
# ---------- Helper: Extract OTP from message ----------
    ...
    for pat in patterns:
        ...
        if m:
            if pat == patterns[-1] and len(m.groups()) == 2:
                ...
                # Skip false positives like long numbers starting with 584, etc.
                if pat == patterns[3] and re.match(r'^(584|1|7|8|9)', m[1]) and len(m[1]) >= 10:
                    continue
                return m[1]
    return None

# ---------- Session Management (exactly like working JS) ----------
def login():
    global session_cookie, sesskey, last_login
    with login_lock:
        # If session is still valid (1 hour), return True
        if session_cookie and (time.time() - last_login) < 3600:
            return True
        try:
            print("[LOGIN] Starting login...")
            # Try both /login and /sign-in (like in JS)
            login_paths = ["/login", "/sign-in"]
            success = False
            for login_path in login_paths:
                login_url = f"{BASE_URL}{login_path}"
                r1 = requests.get(login_url, headers=HEADERS, timeout=10)
                ...
                if r1.status_code != 200:
                    continue
                html = r1.text
                # Extract arithmetic captcha (exact same regex as JS)
                captcha_match = re.search(r'What is (\d+) \+ (\d+) = \?', html)
                if not captcha_match:
                    # If no captcha, try plain POST
                    print("[LOGIN] No captcha found, trying plain POST...")
                    ans = None
                else:
                    ans = int(captcha_match[1]) + int(captcha_match[2])
                    print(f"[LOGIN] Captcha answer: {ans}")

                # Get initial cookie from login page
                current_cookie = get_cookie_from_headers(r1.headers)
                print(f"[LOGIN] Initial cookie: {current_cookie}")

                # Prepare POST to /signin
                ...
                if current_cookie:
                    headers_post["Cookie"] = current_cookie
                r2 = requests.post(
                    signin_url,
                    data=data,
                ...
                # Update cookie from POST response
                ...
                # Check success: redirect 302/301 or 200 with "logout"/"dashboard"
                if r2.status_code in (302, 301):
                    session_cookie = current_cookie
                    last_login = time.time()
                    print(f"[LOGIN] Success ({r2.status_code}) with cookie: {session_cookie}")
                    success = True
                    break
                    ...
                    if "logout" in html2 or "dashboard" in html2:
                        session_cookie = current_cookie
                        last_login = time.time()
                        print(f"[LOGIN] Success (200) with cookie: {session_cookie}")
                        success = True
                        break
            if not success:
                raise Exception("All login paths failed")

            # Wait and fetch sesskey
            time.sleep(0.5)
            # Fetch sesskey from SMSCDRStats page (same as JS)
            r3 = requests.get(
            ...
            # Validate session by fetching one record
...
def validate_session():
    try:
        ...
        if resp.status_code in (503, 403):
            print(f"[VALIDATE] {resp.status_code}, waiting 3s...")
            time.sleep(3)
...
def fetch_numbers():
    ...
    try:
        resp = requests.get(
            ...
            headers={**HEADERS, "Cookie": session_cookie, "Referer": f"{BASE_URL}/agent/MySMSNumbers2"},
            params=params,
            timeout=15
        )
...
# ---------- Fetch ALL OTPs (iDisplayLength = -1) ----------
def fetch_all_otps():
    if not ensure_session():
        return []
    ...
    try:
        resp = requests.get(
            ...
            headers={**HEADERS, "Cookie": session_cookie, "Referer": f"{BASE_URL}/agent/SMSCDRStats"},
            params=params,
            timeout=30
        )
        if resp.status_code != 200:
...
# ---------- Cached OTPs (short TTL to reduce panel load) ----------
def get_cached_otps():
    with cache_lock:
        now = time.time()
        if cache["data"] and (now - cache["timestamp"]) < 30:   # 30 seconds cache
            return cache["data"]
...
def root():
    return jsonify({
        "message": "NumberPanel API (Python on Vercel) – All OTPs & Numbers",
