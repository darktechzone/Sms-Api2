...
SESSKEY_OVERRIDE = os.environ.get("SESSKEY_OVERRIDE", None)  # e.g., "6686814734"
...
def extract_sesskey_from_html(html):
    """Extract sesskey from HTML using patterns that match the DataTable source URL."""
    # Look for sesskey in ajax source URL (e.g., sesskey=6686814734)
    patterns = [
...
def fetch_sesskey():
    ...
    # Try to get sesskey from the client SMSCDRStats page
    try:
    ...
    # If not found, try the numbers page (it also contains the DataTable source)
    try:
...
def login():
    ...
    try:
        ...
        # Validate session (using client endpoint)
...
def get_cached_otps():
    now = time.time()
    if otp_cache['data'] and (now - otp_cache['timestamp']) < 300:
...
def root():
    return jsonify({
        "message": "NumberPanel API – Client Endpoints, Fixed Sesskey Extraction",
