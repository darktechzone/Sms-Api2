...
# ---------- FULL COUNTRY MAP (copy from Node.js) ----------
COUNTRY_MAP = {
...
# ---------- EXACT OTP EXTRACTION – MATCHES NODE.JS 5 PATTERNS ----------
def extract_otp(text):
    ...
    for pat in patterns:
        ...
        if m:
            # Pattern 5: hyphen/space separated – combine both groups
            if pat == patterns[-1] and len(m.groups()) == 2:
                combined = m.group(1) + m.group(2)
                if 4 <= len(combined) <= 8:
                    return combined
            else:
                # Pattern 4: skip false positives (long numbers starting with 584, 1, 7, 8, 9)
                if pat == patterns[3]:
                    val = m.group(1)
...
# ---------- FETCH OTPs – EXACTLY MATCHES NODE.JS ----------
def fetch_otps(limit=100):
    ...
    params = {
        ...
        "sEcho": "1",
        "iDisplayStart": "0",
        "iDisplayLength": str(limit),   # Match Node.js: 100
        "_": int(time.time() * 1000)
    }
    if sesskey:
        params["sesskey"] = sesskey
    ...
    try:
        ...
        for row in rows:
            ...
            if len(result) >= 10:   # Match Node.js: only return last 10
                break
...
def root():
    return jsonify({
        "message": "NumberPanel API (Python – Fixed OTP)",
