...
# ---------- FULL COUNTRY MAP (same as app (4).js) ----------
COUNTRY_MAP = {
...
def fetch_sesskey(retries=3):
    ...
    for attempt in range(1, retries+1):
            ...
            if resp.status_code == 200:
                ...
                for pat in patterns:
                    ...
                    if m:
                        ...
                        return
                add_log(f"Sesskey not found (attempt {attempt})")
            elif resp.status_code in (503, 403):
                add_log(f"Sesskey {resp.status_code}, waiting 3s...")
                time.sleep(3)
                continue
        ...
        if attempt < retries:
            time.sleep(attempt)
    sesskey = None
    add_log("Sesskey set to None")

def login():
    global session, sesskey, last_login, is_logging_in
    if is_logging_in:
        while is_logging_in:
            time.sleep(0.1)
    ...
    try:
        ...
        for login_path in login_paths:
            try:
                ...
                if r1.status_code in (503, 403):
                    add_log(f"Login page {login_path} {r1.status_code}, waiting 3s")
                    time.sleep(3)
                    continue
                ...
                current_cookie = get_cookie_from_headers(r1.headers)
                if not current_cookie and session.cookies:
                ...
                updated_cookie = get_cookie_from_headers(r2.headers)
                if updated_cookie:
                ...
                if r2.status_code in (302, 301):
                    ...
                    add_log(f"Login success ({r2.status_code})")
                    success = True
                    break
        ...
        # Validate session (as Node.js does)
...
def validate_session():
    try:
        ...
        if resp.status_code in (503, 403):
            add_log(f"Validate {resp.status_code}, waiting 3s")
            time.sleep(3)
            return validate_session()
        if resp.status_code == 200 and resp.json().get("aaData") is not None:
            add_log("Validation OK")
            return True
        add_log("Validation Invalid")
        return False
    except Exception as e:
...
def fetch_with_retry(url, options, retries=3):
    ...
    while attempt < retries:
        try:
            resp = requests.get(url, **options, timeout=20) if options.get('method') == 'GET' else requests.post(url, **options, timeout=20)
            # We'll just use session directly to maintain cookies
            # Better: use session.get/post directly with params/headers
            # I'll rewrite this to use session methods.
            # For clarity, I'll inline the logic in fetch_numbers and fetch_otps.
        except Exception as e:
            pass
    # Actually, to keep it simple, we'll not use this wrapper; we'll implement retry in each fetch.
    # I'll rewrite fetch_numbers and fetch_otps with retry logic directly.
    return None

# Since we want to keep code clean, we'll implement retry in the fetch functions.
# We'll also add circuit breaker.

def fetch_with_circuit_breaker(url, method='GET', headers=None, params=None, data=None):
    global consecutive_failures
    if consecutive_failures >= FAILURE_THRESHOLD:
        add_log(f"Circuit breaker – skipping for {BREAKER_TIMEOUT}s")
    ...
    while attempt < 3:
        try:
            if method.upper() == 'GET':
                ...
                resp = session.post(url, headers=headers, params=params, data=data, timeout=20)
...
def fetch_numbers():
    ...
    if not resp or resp.status_code != 200:
        ...
        raise Exception("Numbers fetch failed")
    data = resp.json()
...
def fetch_otps(limit=10):
    if not ensure_session():
        add_log("ensure_session failed for OTP")
        return []
...
# ---------- CACHE ----------
def refresh_cache():
    global otp_cache, cache_refresh_in_progress
    ...
    try:
        ...
        if fresh is not None:
            ...
            add_log(f"Cache updated with {len(fresh)} OTPs")
    except Exception as e:
        add_log(f"Cache refresh error: {e}")
    finally:
        cache_refresh_in_progress = False

def get_cached_otps():
    now = time.time()
    if otp_cache['data'] and (now - otp_cache['timestamp']) < 300:  # 5 min fallback
        if (now - otp_cache['timestamp']) > 10:
...
def root():
    return jsonify({
        "message": "NumberPanel API (Python – exact match to app (4).js)",
