...
def is_likely_phone(text):
    """Check if a string looks like a phone number (digits, 7+ digits)."""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if extracted:
                ...
                add_log("Sesskey not found in page")
        else:
    ...
    with login_lock:
        ...
        try:
            ...
            if not fetch_sesskey():
                add_log("Could not retrieve sesskey, continuing anyway")
...
def ensure_session():
    ...
    if sesskey is None:
        if fetch_sesskey():
            ...
            add_log("Sesskey missing, forcing re-login")
            return login()
    return True

# ---------- FETCH NUMBERS (auto-detect column) ----------
def fetch_numbers():
...
def root():
    return jsonify({
        "message": "NumberPanel API (Client endpoints, auto‑detect number column)",
